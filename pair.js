...
const BOT_NAME_FANCY = '𝐒𝐀𝐒𝐀 𝐗 𝐌𝐃 𝐌𝐈𝐍𝐈';
...
const config = {
  ...
  RCD_IMAGE_PATH: 'https://i.ibb.co/6cpfG0KB/73582.jpg',
  NEWSLETTER_JID: '120363421675697167@newsletter',
  OTP_EXPIRY: 300000,
  OWNER_NUMBER: process.env.OWNER_NUMBER || '94784167385',
  CHANNEL_LINK: 'https://whatsapp.com/channel/0029Vb7P18V84OmA7pIqw03U',
  BOT_NAME: 'SASA X MD MINI',
  BOT_VERSION: '1.0.0V',
  OWNER_NAME: 'Sasanka Chamuth',
  IMAGE_PATH: 'https://i.ibb.co/6cpfG0KB/73582.jpg',
  BOT_FOOTER: '𝐏𝐎𝐖𝐄𝐑𝐃 𝐁𝐘 𝐒𝐀𝐒𝐀 𝐗 𝐌𝐃 🐍',
  BUTTON_IMAGES: { ALIVE: 'https://i.ibb.co/6cpfG0KB/73582.jpg' }
};
...
function setupCommandHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    let botName = cfg.botName || 'SASA X MD MINI';
...
case 'gpt': {
  try {
    ...
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'SASA X MD MINI AI';
...
case 'aiimg2': {
    ...
    try {
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || 'QUEEN ASHA MINI BOT'
...
END:VCARD`
        ...
        // 🔹 Call API
        const apiUrl = `https://saviya-kolla-api.koyeb.app/download/apk?q=?id=${encodeURIComponent(q)}`;
...
╭─❀ 🤖 ${botName} ❀─╮
│ 🌟 Status   : Online
│ 👑 Owner   : ${config.OWNER_NAME || 'Sasanka Chamutj'}
...
_© Powered by ${botName}_`;
 ...
 case 'video': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'SASA X MD MINI';    // fake contact for quoted card
    const botMention = {
        key: {
            remoteJid: "status@broadcast",
            participant: "0@s.whatsapp.net",
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || 'SASA X MD MINI';
...
`.trim();
    ...
    const defaultImg = 'https://i.ibb.co/6cpfG0KB/73582.jpg';
    ...
    await socket.sendMessage(sender, {
      image: imagePayload,
      caption: text,
      footer: "POWERD BY SASA X MD MINI 😽💕",
      buttons,
      headerType: 4
    }, { quoted: shonux });
...
case 'creative': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || 'SASA X MD MINI AI';
...
END:VCARD` } }
        ...
        const buttonMessage = {
            ...
            footer: config.FOOTER || '> SASA X MD MINI 😽',
            buttons: buttons,
            headerType: 4,
            contextInfo: { mentionedJid: [sender] }
        };
...
//💐💐💐💐💐💐
  ...
  if (!jid) {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || 'SASA MINI BOT AI';
  ...
  if (!(isOwner || isAdmin)) {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || 'SASA X MD MINI BOT AI';
    const shonux = {
...
case 'font': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'SASA X MD MINI BOT'
    // 🔹 Fake contact for Meta AI mention
    const botMention = {
        key: {
            remoteJid: "status@broadcast",
...
case 'mfdl': {
    try {
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || 'SASA MINI BOT AI';
...
END:VCARD`
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || 'SASAMINI BOT AI';
...
case 'cid': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'SASA X MD MINI BOT AI';
...
case 'owner': {
  try {
    ...
    let vcard = 
      ...
      'TITLE:Founder & CEO of sasa x md Mini Bot;\n' + // Title / Role
      'EMAIL;type=INTERNET:support@gmail.com\n' + // Email
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || 'SASA MINI BOT AI'; // dynamic bot name
...
case 'setbotname': {
  ...
  if (!name) {
    ...
    return await socket.sendMessage(sender, { text: '❗ Provide bot name. Example: `.setbotname SASA MINI - 01`' }, { quoted: shonux });
  }
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        try {
          ...
          try { exec(`pm2.restart ${process.env.PM2_NAME || 'SASA-MINI-main'}`); } catch(e) { console.error('pm2 restart failed', e); }
        }
      }
      if (connection === 'close') {
        try { if (fs.existsSync(sessionPath)) fs.removeSync(sessionPath); } catch(e){}
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FANCY, message: '🇱🇰SASA  FREE BOT', activesession: activeSockets.size });
});
...
process.on('uncaughtException', (err) => {
  ...
  try { exec(`pm2.restart ${process.env.PM2_NAME || 'SASA-MINI-main'}`); } catch(e) { console.error('Failed to restart pm2:', e); }
});
